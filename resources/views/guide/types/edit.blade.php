<x-app-layout>
    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 bg-white border-b border-gray-200">
                        <form method="post" action="{{ route('types.update') }}" enctype="multipart/form-data">
                            @csrf
      ...
      " id="name" name="name" required value="{{ Arr::exists($data, 'name') ? $data->name : '' }}"/>
                            ...
                            <a href="{{ route('types.list') }}" class="border-solid border border-gray-500 rounded bg-red-100 hover:bg-red-200 p-2">Отменить</a>
                        </form>
                </div>
            </div>
        </div>
