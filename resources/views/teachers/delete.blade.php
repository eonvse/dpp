<x-app-layout>
    <x-slot name="header">
        <h2 class="text-center sm:text-left font-semibold text-xl text-gray-800 leading-tight">
            <a href="{{ route('teachers.list')}}" class="px-2 py-1 text-sky-800 hover:text-sky-500 hover:underline hover:underline-offset-8">{{ __('Педагоги') }}</a>
            {{ __('Удаление') }}
        </h2>
    </x-slot>

    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 bg-white border-b border-gray-200">
                        <form method="post" action="{{ route('teachers.delete.confirm') }}" enctype="multipart/form-data">
                            @csrf
                            @if (!empty($data->idUser))
                            <input type="hidden" id="idUser" name="idUser" value="{{ $data->idUser }}">
                            @endif
                            <input type="hidden" id="id" name="id" value="{{ $data->id }}">
                            <table class="table-auto my-3">

                                <tr>
                                    <td class="block md:table-cell italic font-light px-2">Фамилия</td>
                                    <td class="block md:table-cell px-2">
                                        {{ $data->surname }}
                                    </td>
                                </tr>
                                <tr>
                                    <td class="block md:table-cell italic font-light px-2">Имя</td>
                                    <td class="block md:table-cell px-2">
                                        {{ $data->name }}
                                    </td>
                                </tr>
                                @if (!empty($data->patronymic))
                                <tr>
                                    <td class="block md:table-cell italic font-light px-2">Отчество</td>
                                    <td class="block md:table-cell px-2">
                                        {{ $data->patronymic }}
                                    </td>
                                </tr>
                                @endif
                                @if (!empty($institutions))
                                <tr>
                                    <td class="block md:table-cell italic font-light px-2">Учреждение</td>
                                    <td class="block md:table-cell px-2">
                                        {{ $institutions->name }}
                                    </td>
                                </tr>
                                @endif                                
                                @if (!empty($positions))
                                <tr>
                                    <td class="block md:table-cell italic font-light px-2">Должность</td>
                                    <td class="block md:table-cell px-2">
                                        {{ $positions->name }}
                                    </td>
                                </tr>
                                @endif                                
                            </table>
                            @if ($data->courses->count()>0) 
                                 <div class="text-red-400 m-2 font-bold">У педагога есть заполненные курсы. <br>Удалите все курсы педагога для удаления самого педагога.</div>
                            @else
                                <button type="submit" class="border-solid border border-gray-500 rounded bg-red-100 hover:bg-red-200 p-2">Удалить</button>
                            @endif
                            <a href="{{ route('teachers.list') }}" class="border-solid border border-green-500 rounded bg-green-100 hover:bg-green-200 p-2">Отменить</a>
                        </form>
                </div>
            </div>
        </div>
    </div>
</x-app-layout>