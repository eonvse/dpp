<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            ...
            <div class="">
                <button type="button" class="inline-flex items-center px-4 py-2 bg-gray-800 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-gray-700 focus:bg-gray-700 active:bg-gray-900 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 transition ease-in-out duration-150" data-bs-toggle="modal" data-bs-target="#addCourseType">Добавить
                </button>
            </div>

            <div class="modal fade fixed top-0 left-0 hidden w-full h-full outline-none overflow-x-hidden overflow-y-auto" id="addCourseType" tabindex="-1" aria-labelledby="addCourseType" aria-modal="true" role="dialog">
                    ...
                    <div class="w-full flex sm:grid sm:grid-cols-2">
                        ...
                        <div class="w-full flex sm:grid sm:grid-cols-2 odd:bg-white even:bg-slate-100 border border-slate-200">
                            ...
                            <div class="flex flex-1 text-center mx-auto">
                                <x-link-edit :href="route('courseTypes.edit', $item->id)" :title="$item->name.' Редактировать'"></x-link-edit>
                                <x-link-del :href="route('courseTypes.delete', $item->id)" :title="$item->name.' Удалить'"></x-link-del>
                            </div>
                        </div>
                        @endforeach
                    </div>
