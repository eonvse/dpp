<x-app-layout>
    <x-slot name="header">
        <h2 class="text-center sm:text-left font-semibold text-xl text-gray-800 leading-tight">
            <a href="{{ route('courses.personal')}}" class="px-2 py-1 text-sky-800 hover:text-sky-500 hover:underline hover:underline-offset-8">{{ __('Мои курсы') }}</a>{{ __('Удаление') }}
        </h2>
    </x-slot>
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 bg-white border-b border-gray-200">
                        <form method="post" action="{{ route('courses.personal.delete.confirm') }}" enctype="multipart/form-data">
                            @csrf
                            ...
                            <a href="{{ route('courses.personal') }}" class="border-solid border border-green-500 rounded bg-green-100 hover:bg-green-200 p-2">Отменить</a>
                        </form>
                </div>
            </div>
        </div>
