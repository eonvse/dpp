<x-app-layout>
    <x-slot name="header">
        <div class="font-semibold text-xl text-gray-800 leading-tight">
            <h2 class='inline-block'>{{ __('Мои курсы') }}</h2>      
            <div class="flex sm:inline-block">
                <a href="{{ route('courses.personal.create') }}" class="inline-flex items-center px-4 py-2 bg-gray-800 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-gray-700 focus:bg-gray-700 active:bg-gray-900 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 transition ease-in-out duration-150">{{ 'Добавить' }}</a>
            </div>
        </div>
    </x-slot>
    <div class="text-gray-800 leading-tight block max-w-7xl mx-auto my-2">
        ...
        <a href="{{ route('courses.personal') }}" class="w-full block text-center sm:w-auto sm:flex sm:inline-flex sm:items-center px-2 py-1 my-2 bg-white border border-gray-300 rounded-md font-semibold text-base text-gray-700 tracking-widest shadow-sm hover:bg-rose-200 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 disabled:opacity-25 transition ease-in-out duration-150">
            Сбросить
        </a>        
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="max-w-7xl">
                    <div class="text-sm text-gray-600 w-full sm:grid sm:grid-cols-12">
                        <div class="col-span-2 font-bold"><span class="hover:text-sky-500">@sortablelink('fullName','Наименование ОУ')</span></div>
                        ...
                        <div class="col-span-2 font-bold text-center">
                            ...
                            <span class="hover:text-sky-500">@sortablelink('dateDoc','Дата получения')</span>
                        </div>
                        <div class="font-bold text-center">Часов</div>
                        <div class="text-xs font-bold text-right">Автор/Редактор<br>
                            <span class="hover:text-sky-500">@sortablelink('updated_at','Дата заполнения')</span>
                        </div>
                    ...
                    <div class="text-xs w-full sm:grid sm:grid-cols-12 odd:bg-white even:bg-slate-100 border border-slate-200">
                        <div class="col-span-2"><span title="{{ $item->fullName }}">{{ Str::substr($item->fullName,0,80) }}</span></div> 
                        ...
                        <div class="flex flex-1 text-center mx-auto">
                            <x-link-edit :href="route('courses.personal.edit', $item->id)" :title="'Редактировать '.$item->progName"></x-link-edit>
                            <x-link-del :href="route('courses.personal.delete', $item->id)" :title="'Удалить '.$item->progName"></x-link-del>
                        </div>
                    </div>
                    @endforeach
                    <div>
